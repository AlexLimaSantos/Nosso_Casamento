...
export default function RSVPPage() {
  ...
  return (
    <div className="min-h-screen bg-fundo font-sans flex flex-col relative overflow-hidden">
      <Navbar />

      <main className="flex-grow max-w-2xl w-full mx-auto px-4 pt-32 pb-20">
        <div className="bg-white rounded-[2rem] shadow-sm border border-casamento/10 overflow-hidden text-center">
          
          <div className="bg-casamento py-6 px-4">
            <h2 className="font-casamento text-4xl text-white">Confirme sua Presença</h2>
          </div>
          
          {carregandoDados ? (
            <div className="p-12 flex flex-col items-center justify-center">
              <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-casamento mb-4"></div>
              <p className="text-casamento font-bold">Buscando seu convite...</p>
            </div>
          ) : (
            <div className="p-8 md:p-10">
              {!sucesso ? (
                <form onSubmit={handleEnviarRSVP}>
                  <div className="mb-8 bg-fundo p-4 rounded-xl border border-gray-100">
                    <p className="text-gray-500 text-sm uppercase tracking-widest mb-1 text-center">Convite em nome de</p>
                    <h3 className="text-2xl font-bold text-casamento text-center">{nomeConvidado}</h3>
                  </div>

                  <div className="mb-8">
                    <label className="block text-gray-700 font-bold mb-4 text-lg text-center">Você poderá comparecer?</label>
                    <div className="flex flex-col sm:flex-row gap-4">
                      <label className={`flex-1 flex items-center justify-center p-4 rounded-xl border-2 cursor-pointer transition-all ${vaiComparecer === "sim" ? "border-casamento bg-casamento/5" : "border-gray-200"}`}>
                        <input type="radio" value="sim" checked={vaiComparecer === "sim"} onChange={(e) => setVaiComparecer(e.target.value)} className="hidden" />
                        <span className={`font-bold ${vaiComparecer === "sim" ? "text-casamento" : "text-gray-400"}`}>✅ Sim, estarei lá!</span>
                      </label>
                  ...
                  {vaiComparecer === "sim" && limiteAcompanhantes > 0 && (
                    <div className="mb-8 text-left animate-in fade-in duration-300">
                      ...
                      <select value={acompanhantesConfirmados} onChange={(e) => handleMudancaQuantidade(Number(e.target.value))} className="w-full px-4 py-3 rounded-lg border border-gray-300 mb-5 bg-white focus:ring-casamento focus:border-casamento">
                      ...
                      {acompanhantesConfirmados > 0 && (
                        <div className="space-y-3 bg-gray-50 p-4 rounded-xl border border-gray-200">
                           <p className="font-bold text-casamento mb-3 text-xs uppercase tracking-widest text-center">Nomes:</p>
                           {nomesAcompanhantes.map((nome, index) => (
                             <input key={index} type="text" required value={nome} onChange={(e) => handleNomeAcompanhanteChange(index, e.target.value)} placeholder={`Nome completo do ${index + 1}º acompanhante`} className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-casamento focus:outline-none" />
                           ))}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="mb-8 text-left">
                    ...
                    {/* Placeholder dinâmico puxando do CONFIG */}
                    <textarea rows={3} value={mensagemRSVP} onChange={(e) => setMensagemRSVP(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 resize-none focus:ring-2 focus:ring-casamento focus:outline-none" placeholder={`Escreva algo para ${CONFIG.casal.noivo} e ${CONFIG.casal.noiva}...`}></textarea>
                  </div>

                  <button type="submit" disabled={enviandoRSVP} className="w-full bg-casamento text-white font-bold py-4 rounded-xl hover:opacity-90 transition shadow-md disabled:opacity-70 active:scale-95">
                ...
                <div className="py-8 animate-in zoom-in duration-300">
                  ...
                  <p className="text-gray-600 mb-8 text-lg">Obrigado, {nomeConvidado}! Sua resposta foi salva no nosso planejamento para o grande dia.</p>
                  
                  <div className="flex flex-col gap-3">
                    <button onClick={() => window.location.href = '/inicio'} className="w-full bg-casamento text-white font-bold py-3 rounded-xl shadow-md hover:opacity-90 transition">Voltar ao Início</button>
      ...
      {toast.visivel && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[200] animate-in slide-in-from-bottom-5 duration-300">
          <div className={`bg-white border-l-4 ${toast.tipo === 'sucesso' ? 'border-green-500' : 'border-casamento'} shadow-2xl rounded-xl px-6 py-4 flex items-center gap-3 min-w-[300px]`}>
            <span className={`${toast.tipo === 'sucesso' ? 'text-green-500 bg-green-50' : 'text-casamento bg-casamento/10'} p-2 rounded-full`}>
              {toast.tipo === 'sucesso' ? (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-5 h-5 text-casamento">
